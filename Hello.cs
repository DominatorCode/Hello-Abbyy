...
namespace Hello
{
    public class HelloForm : System.Windows.Forms.Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            this.label1.Text = "Óêàæèòå ïàïêó äëÿ ýêñïîðòà";
            // 
            // HelloForm
            // 
        ...
        private void goButton_Click(object sender, System.EventArgs e)
        {
            ...
            try
            {
                ...
                //RunExternalExe(@"E:\VS Pro\DocNet Visual Editor v12\bin\Release\DocNet Visual Editor v12.exe", @"E:\VS Pro\Hello (C#) v12\bin\Debug\HelloFiles\FCEExport\Àêò2018-10-31_11-35-08.tif");
        ...
        public void Scanning()
        {
            ...
            string sourcesList = "Ñïèñîê ñêàíåðîâ: ";
            ...
            {
                sourcesList = "Ñêàíåðû íå íàéäåíû";
            }
            trace(sourcesList);
        ...
        private void processImages()
        {
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    if (document == null)
                    {
                        ...
                        ImageFileFormatEnum.IFF_Tif, null, ImageCompressionTypeEnum.ICT_CcittGroup4, null);
                            _listErrorsDocumentRecognizing.Add("Íå óäàëîñü ðàñïîçíàòü èçîáðàæåíèå: " + document.Pages[0].OriginalImagePath);
                            continue;
                        }
                    }
                    ...
                    // åñëè èñïîëüçóåòñÿ âñòðîåííûé ñïîñîá îáðàáîòêè èçîáðàæåíèé,
                    // ïðîâåðèòü èõ êà÷åñòâî
                    if (!condUsingCustomImageSource)
                        CheckIsImageSuitableForOcr(document);
                    // ñåðèàëèçàöèÿ äàííûõ â îòäåëüíóþ ïàïêó
                ...
                // äîáàâëÿåì èíôîðìàöèþ î íåðàñïîçíàííûõ ñòðàíèöàõ
                ...
                // äîáàâëÿåì èíôîðìàöèþ î íåðñàïîçíàííûõ ëèñòàõ äåêëàðàöèè
                if (listDeclarationRecognizeErrors.Count > 0)
                {
                    if (String.IsNullOrEmpty(_warningRecognizeResults))
                        _warningRecognizeResults = string.Join(System.Environment.NewLine, listDeclarationRecognizeErrors);
        ...
        /// <summary>Èíèöèàëèçèðóåò äâèæîê ðàñïîçíàâàíèÿ ABBYY</summary>
        /// <returns>Èñòèíà - äâèæîê çàãðóæåí</returns>
        public bool LoadEngine()
        {
            ...
            try
            {
                ...
                traceText += " ,çàãðóæàåì äâèæîê _engineLoader.Load()";
                ...
                {
                    errorText = "Äàííàÿ ëèöåíçèÿ íå àêòèâèðîâàíà, ëèáî âñòàâëåí íåâåðíûé êëþ÷.";
                    //MessageBox.Show(errorText);
                    UnloadEngine();
                    return false;
                }

                // åñëè èñïîëüçóåòñÿ íåñêîëüêî ëèöåíçèé ïîäáèðàåì òó
                // ó êîòîðîé åñòü ñòðàíèöû
                if (license.RemainingUnits == 0)
                {
                    if (!ChangeLicense())
                    {
                        errorText = "Òåêóùàÿ ëèöåíçèÿ èìååò 0 ñòðàíèö äëÿ ðàñïîçíàâàíèÿ, íå óäàëîñü ñìåíèòü ëèöåíçèþ íà äðóãóþ";
                    ...
                    case -2147221164:
                        {
                            ...
                            errorText = errorText + Environment.NewLine + "Íåîáõîäèìî âûïîëíèòü ðåãèñòðàöèþ ïðîãðàììû (regsvr) â ñëóæáå êîìïîíåíò";
                            //MessageBox.Show(errorText);
                            break;
                        }
                    default:
            ...
            catch (Exception e)
            {
                if (e.Message.Contains("80070005"))
                {
                    ...
                    errorText = errorText + Environment.NewLine + "Íåîáõîäèìî ðàñøèðèòü ïðàâà äîñòóïà äëÿ ïðîãðàììû â ñëóæáå êîìïîíåíò";
                    //MessageBox.Show(errorText);
            ...
            traceText += " ,äâèæîê çàãðóæåí";

            return true;
        }

        /// <summary>Âûãðóæàåò äâèæîê ðàñïîçíàâàíèÿ ABBYY â ñèñòåìå</summary>
        ...
        // äîáàâëåíèå äîïîëíèòåëüíîé ÿ÷åéêè íà 3é óðîâåíü äëÿ êîððåêòíîãî ÷òåíèÿ äîêóìåíòà
            ...
            foreach (string xmlFile in xmlFiles)
            {
                ...
                if (itemNodes.Count > 0)
                {
                    ...
                    for (int i = itemNodes.Count - 1; i >= 0; i--)
                    {
                        ...
                        else if (condTaxField) // âû÷èñëÿåì ÍÄÑ âõîäèò â ñòîèìîñòü èëè íåò
                        {                           
                            double Price = 0;
                            double Qty = 0;
                            double Cost = 0;
        ...
        string SetDeclarationFileName(string nameDocument, string numberPage, string pathFolderExport)
        {
            ...
            else
            {
                ...
                {
                    _listErrorsDocumentRecognizing.Add("Ëèñòû äëÿ äåêëàðàöèè âîçìîæíî ïåðåïóòàíû");
        ...
        bool ChangeLicense()
        {
            ...
            if (listLicenses.Count > 1)
            {
                ...
                if (listCountReminingUnits.Max() > 0)
                {
                    ...
                    {
                        _errorText = "Íå óäàëîñü ñìåíèòü ëèöåíçèþ: " + Ex.Message;
        ...
        {
            // åñëè ó ÊÄÒ îòñóòñòâóþò ñâîè ëèñòû
            // êîïèðóåì ó ÃÄÒ
        ...
        public bool RunExternalExe(string pathExe, string pathFile)
        {
            ...
            {
                _errorText = "Îøèáêà çàïóñêà âíåøíåãî exe: " + ex.Message + Environment.NewLine + ex.StackTrace;
                throw new Exception(_errorText);
            }
        }
        ...
        {
            if (_engine == null)
                throw new Exception("Äâèæîê FlexiCapture íå èíèöèàëèçèðîâàí");
            ...
            if (Path.GetExtension(pathFileTemplate) == ".cfl")
            {
                ...
                return collectionClasses.ToString() + " âåðñèÿ ";*/
                return "";
            }
        ...
        public string GetTemplateVersion()
        {
            ...
            {
                _errorText = "ERROR: FC_Recognize: GetTemplateVersion: " + Environment.NewLine + "Íå çàäàí ïóòü ê äèðåêòîðèè ñ øàáëîíàìè";
                throw new Exception(_errorText);
            }
        ...
        public void SetTemplatesDirectory(string pathDirectoryTemplates)
        {
            ...
            if (!Directory.Exists(pathDirectoryTemplates))
                throw new Exception("Äèðåêòîðèÿ îòñóòñòâóåò: " + pathDirectoryTemplates);
        ...
        public bool SetLoggingPath(string pathLogging)
        {
            ...
            else
            {
                ...
                {
                    AppParameters.TextError.Add("SetLoggingPath: Íå óäàëîñü ñîçäàòü äèðåêòîðèþ: " + pathDirectoryFileLogging + Environment.NewLine + ex.Message);
                    return false;
                }
            }
            ...
            {
                AppParameters.TextError.Add("Íåò ïðàâ íà çàïèñü ó äèðåêòîðèè: " + pathDirectoryFileLogging);
                return false;
            }
        ...
        {
            AppParameters._listInfoQualityImages.Add("Íåîáõîäèìî íàñòðîèòü ñêàíåð äëÿ ïîâûøåíèÿ òî÷íîñòè ðàñïîçíàâàíèÿ äîêóìåíòîâ");
        ...
        /// <summary>Ïðîâåðÿåò, ÷òî èçîáðàæåíèå íå öâåòíîå è èìååò ðàçðåøåíèå 300 dpi</summary>
        /// <param name="pDocument">Ðàñïîçíàííûé äîêóìåíò</param>
            ...
            if (AppParameters._isSuitableForOcr)
            {
                if (String.IsNullOrEmpty(AppParameters._warnResolutionImageBad))
                    if (pDocument.Pages[0].Image.SourceImageXResolution < 300 || pDocument.Pages[0].Image.SourceImageYResolution < 300)
                    {
                        ...
                        AppParameters._warnResolutionImageBad = "Îïòèìàëüíîå ðàçðåøåíèå ñêàíåðà äëÿ ðàñïîçíàâàíèÿ äîëæíî áûòü îò 300 dpi" + Environment.NewLine +
                           "Òàê æå ïðîâåðüòå öâåòîâîé ðåæèì ñêàíåðà, îïòèìàëüíûé - îòòåíêè ñåðîãî èëè ÷åðíî-áåëûé";
                    }                                          
            }
        }
